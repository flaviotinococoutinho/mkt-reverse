...
## 2024-05-25 - Dynamic Inline Error Messages ARIA Attributes
**Learning:** In the frontend React application (`frontend/mkt-reverse-web`), inline error messages (often styled with the `errorInline` class) lack accessibility attributes by default.
**Action:** Ensure inline error messages include `role="alert"` and `aria-live="assertive"` to properly announce asynchronous validation or submission failures to screen readers.
