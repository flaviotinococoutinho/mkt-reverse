...
**Learning:** Found that inline error messages (e.g., styled with `.errorInline` indicating async submission or validation failures) lacked ARIA attributes, meaning screen reader users wouldn't be notified when an error appeared dynamically.
**Action:** When creating or modifying inline error messages that appear asynchronously, always ensure they include `role="alert"` and `aria-live="assertive"` so that screen readers announce the failure immediately.
