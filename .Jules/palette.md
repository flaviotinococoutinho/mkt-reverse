...
## 2024-05-24 - Accessibility of Inline Error Messages
**Learning:** Found that inline error messages (`.errorInline` elements) used to display validation or submission errors were missing accessibility attributes, making it difficult for screen readers to announce asynchronous failures.
**Action:** Always add `role="alert"` and `aria-live="assertive"` to inline error message containers so screen readers properly announce asynchronous validation or submission errors to users without requiring them to shift focus manually.
