...
**Learning:** Found that inline error messages (`errorInline`) displaying asynchronous validation or submission failures lacked `role="alert"` and `aria-live="assertive"`, meaning they were not properly announced to screen reader users when they appeared dynamically.
**Action:** Always add `role="alert"` and `aria-live="assertive"` to elements dynamically displaying error messages to ensure they are immediately read by assistive technologies.
