...
### Added
- `docs/product/roadmap.md` — plano mestre de produto: diagnóstico por camada e por jornada (21 jornadas com veredito do código), lacunas para ser um bom produto e as mecânicas que o tornam interessante (linha do tempo do contrato, link público do "quero", radar de demanda, reputação por contrato, preço-âncora), métricas/North Star, quatro releases (R1 Loop fechado → R2 Confiança & retenção → R3 Dinheiro real → R4 Crescimento) com histórias e critérios de aceite, lacunas de API, riscos e definição de pronto
...
### Changed
- Planejamento reorganizado em duas camadas: `roadmap.md` (o quê/por quê) e `next-fronts.md` reescrito como execução técnica por release (o como); README, `business-model.md` §6 e `vision.md` apontam para o roadmap e mapeiam fases × releases; `.github/copilot-instructions.md` inclui o roadmap na ordem de leitura
